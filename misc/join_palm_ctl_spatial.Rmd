...
```{r}
# palm_obj <- readRDS(file = here::here(glue::glue()))
palm_obj <- "{map_27}/{robj_dir}/se_deconv_uzntl1_fkclxp_epid20.rds" %>%
# "{map_27}/{robj_dir}/se_deconv_{sample_id}.rds" %>%
  glue::glue() %>%
  here::here() %>%
  readRDS(file = .)

# ct_obj <- readRDS(file = here::here(glue::glue("{anot_27}/{robj_dir}/clust_se_k886qc_rqix54.RDS")))
ct_obj <- "{map_27}/{robj_dir}/se_deconv_k886qc_rqix54_epid20.rds" %>%
# "{map_27}/{robj_dir}/se_deconv_{sample_id}.rds" %>%
  glue::glue() %>%
  here::here() %>%
  readRDS(file = .)
...
expr_palm_df <- data.frame(expr_palm) %>% tibble::rownames_to_column("gene")
expr_ct_df <- data.frame(expr_ct) %>% tibble::rownames_to_column("gene")
...
saveRDS(object = expr_mtrx,
        file = here::here("misc/expr_mtrx_palm-control.RDS"))
```
...
saveRDS(object = metadata_df,
        file = here::here("misc/metadata_palm-control.rds"))
```
