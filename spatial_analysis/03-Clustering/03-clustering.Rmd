...
dir.create(path = here::here(sprintf("%s/%s", clust_27, plt_dir)),
           showWarnings = FALSE,
           recursive = TRUE)

dir.create(path = here::here(sprintf("%s/%s", clust_27, robj_dir)),
           showWarnings = FALSE,
           recursive = TRUE)
```
...
```{r}
se_obj <- readRDS(file = here::here(sprintf("%s/%s/qc_se_%s.rds",
                                            qc_27, robj_dir, sample_id)))
```
...
saveRDS(object = se_obj,
        file = here::here(sprintf("%s/%s/clust_se_%s.RDS",
                                  clust_27, robj_dir, sample_id)))
```
