...
```{r}
spaceranger_data <- sprintf("%s/%s/outs",
                           spaceranger, sample_id)
...
## Filtering spots
```{r}
if (sample_id == "s2yq8o_t5hk8u") {
  qc_message <- "We need to filter out spots overlapping the tissue."
} else {
  qc_message <- "No need to fitler out any bad quality spots."
  }
```
...
```{r eval = FALSE}
if (sample_id == "s2yq8o_t5hk8u") {
  readr::read_table(file = here::here(glue::glue("{qc_27}/{robj_dir}/")))
  coord <- se_obj@images$s2yq8o_t5hk8u@coordinates[, c("imagerow", "imagecol")]
  
  mask <- which(coord$imagerow > 200 & coord$imagerow < 3000)
  keep_spots <- rownames(coord[mask, ])
  se_obj <- se_obj[, keep_spots]
  
  plotly::plot_ly(coord, x = ~imagecol, y = ~imagerow)
}
```
...
saveRDS(object = se_obj,
        file = here::here(sprintf("%s/qc_se_%s.rds",
                                  robj_dir, sample_id)))
```
